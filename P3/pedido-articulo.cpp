...
void Pedido_Articulo::pedir(Pedido& p, Articulo& a, double pr, size_t cant){
	pa[&p].insert(std::make_pair(&a,LineaPedido(pr,cant)));	
	ap[&a].insert(std::make_pair(&p,LineaPedido(pr,cant)));

}

void Pedido_Articulo::pedir(Articulo& a, Pedido& p, double pr, size_t cant){
