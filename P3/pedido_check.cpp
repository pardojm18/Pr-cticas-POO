/* $Id: pedido_check.cpp 394 2017-04-28 17:33:06Z gerardo $
 * pedido_check.cpp -- Comprueba el código fuente de los ficheros de la P3
 * ©2016-17 Pedro Delgado y los profesores de POO
 */
#include "checkCode.h"
...
int main(int argc, const char **argv){
  ...
  if(c1.findClass("Articulo")){
    ...
    c1.includedHeader(functionNames, headerName, true, "Revisa de dónde son tomadas las funciones de la biblioteca estándar como strlen, strcpy...");

    c1.memberVariableAccessLevel("Articulo", {".*"}, "private", "Revisa el acceso a los atributos.");
  ...
  if(c2.findClass("Tarjeta")){
    ...
    c2.memberVariableAccessLevel("Tarjeta", {".*"}, "private", "Revisa el acceso a los atributos.");
  ...
  if(c3.findClass("Usuario")){
    ...
    c3.memberVariableAccessLevel("Usuario", {".*"}, "private", "Revisa el acceso a los atributos.");
  ...
  if(c4.findClass("Pedido")){
    ...
    c4.memberVariableAccessLevel("Pedido", {".*"}, "private", "Revisa el acceso a los atributos.");
    ...
    c4.function({"operator<<"}, {{"?"}}, "Revisa el lugar de la declaración de los operadores.");

    c4.memberVariable("Pedido", {"tarjeta_"}, {"const"}, "Revisa el enunciado respecto al atributo de la tarjeta de pago.");
  ...
  if(c5.findClass("Pedido_Articulo")){
    ...
    c5.defaultArgumentsInMethod({"LineaPedido"}, methodsParams, "Pedido", {"?"}, {1}, {{"1u"}}, "Revisa el enunciado respecto a la construcción de objetos.");
    ...
    c5.function({"operator<<"}, {{"?"}}, "Revisa el lugar de la declaración de los operadores.");

    //'pedir' sobrecargado puede hacerse mediante la busqueda de dos metodos con diferentes parametros.
    c5.method({"pedir", "pedir"}, {{"class Pedido &", "class Articulo &", "double", "unsigned int"}, {"class Articulo &", "class Pedido &", "double", "unsigned int"}}, "Pedido_Articulo", {"?", "?"}, "Se sugiere la sobrecarga del método 'pedir'.");
  ...
  if(c6.findClass("Usuario_Pedido")){
    ...
    c1.memberVariableAccessLevel("Usuario_Pedido", {".*"}, "private", "Revisa el acceso a los atributos.");
