...
istream& operator >> (istream &i, Fecha& a){
	/*char cad [12] = {'\0'};
	i >> cad;
...
bool validaIs(int& dia, int& mes, int& anno){
	...
	if (anno > Fecha::AnnoMaximo)
		return false;
	if (anno < Fecha::AnnoMinimo)
		return false;
	if(mes < 1 or mes > 12)
		return false;
	if(dia < 1 or dia > dias[mes - 1]){
		return false;
	}
...
char* Fecha::cadena () const{
	static char cad[sizeof("miércoles DD de septiembre de AAAA")];
 	const char* DIASEM = {NULL};
 	const char* nombreMes = {NULL};
